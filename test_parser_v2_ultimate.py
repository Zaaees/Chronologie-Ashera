...
from unify_characters_v2 import get_canonical_name_v2, build_unified_characters_dict_v2, CHARACTER_METADATA_V2, CANONICAL_MAP

# Add missing handle mappings to CANONICAL_MAP
CANONICAL_MAP["alieny"] = "Jasp Nah"
CANONICAL_MAP["alieny_"] = "Jasp Nah"
CANONICAL_MAP["jean hazagi"] = "Inzu Sravel"
...
# --- CORRECTION 2: Enhanced HRP & MJ Instruction Cleaning ---
HRP_EXCL_PATTERNS = [
...
def clean_message_content_ultimate(content):
    if not content:
        return ""
    text = str(content).strip()
    
    # 1. Remove Discord user/role pings <@123>
    text = re.sub(r'<@[!&]?\d+>', '', text)
    
    # 2. Remove Tupperbot ping headers like '@> Ju | Lucia ...'
    text = re.sub(r'^\s*@>\s*[^\n]+', '', text)
    
    # 3. Remove text user pings like @RED, @Nyx, @Khem, @Brutus
    text = re.sub(r'@[A-Za-z0-9_\-\'\"]+', '', text)
    
    # 4. Remove MJ systemic instructions in parentheses (e.g. "(Vous avez jusqu'à...)" or "( Vous avez, à partir d'ici...)")
    text = re.sub(r'\([^)]*?(?:possibilités|répondre|mission|tours|objectif|décision)[^)]*?\)', '', text, flags=re.IGNORECASE)
    
    # 5. Remove empty codeblocks ``` ```
    text = re.sub(r'```\s*```', '', text)
    
    # 6. Apply HRP inline regex
    cleaned = HRP_INLINE_REGEX.sub('', text).strip()
    cleaned = re.sub(r'\n{3,}', '\n\n', cleaned).strip()
    return cleaned

def is_meaningful_rp_content_ultimate(content):
    if not content:
        return False
    text = str(content).strip()
    if not text:
...
def create_scene_object_ultimate(channel_clean, channel_raw, scene_index, message_tuples, title_suggested):
    rp_msgs = [m for m in message_tuples if is_meaningful_rp_content_ultimate(m[1])]
    if not rp_msgs:
    ...
    for msg, text in rp_msgs:
        ...
        cleaned_txt = clean_message_content_ultimate(text)
...
def run_simulation():
    ...
    for sc in existing_scenes:
        ...
        if msgs:
            for m in msgs:
                ...
                if is_meaningful_rp_content_ultimate(full_text):
    ...
    for ch_clean, ch_info in channels_map.items():
        ...
        ch_scenes = segment_messages_into_scenes_v2(
            ...
            ch_raw,
            valid_msgs,
            create_scene_object_ultimate
        )
        
        # Standalone micro-scenes threshold >= 3
    ...
    with open('scenes_fixed_ultimate.json', 'w', encoding='utf-8') as f:
        json.dump(sim_data, f, ensure_ascii=False, indent=2)

    print(f"Simulation ULTIMATE terminée : {len(all_v2_scenes)} scènes générées dans 'scenes_fixed_ultimate.json'.")
